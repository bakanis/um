...
type UserManager interface {
	...
	// If any error occures (such as database error or duplicate email),
	// the returning User will be nil along with the error.
	// The email address will be converted to lower case.
	CreateUser(emailAddr string, displayName string, status int32) (User, error)
	...
	// Find looks for a User with a matching email address.
	...
	// EmailAddrExists returns true iff the user name already exists in the database.
	// This operation is case insensitve.
	// If an error occurs during the operation, false will be return along with the error.
	EmailAddrExists(emailAddr string) (bool, error)
